...
def pdb_reader (traj_file):
    ...
    for j,line in enumerate(f):
        if line.startswith("ATOM") or line.startswith("HETATM"):
            ...
            atom_info['atom_serial_number'] = int(line[6:11].strip())
            ...
            atom_info['residue_sequence_number'] = int(line[22:26].strip())
...
def gro_reader (traj_file,frame=1):
    ...
    for line in lines[(frame-1)*(atom_nums+3)+2:(frame-1)*(atom_nums+3)+2+atom_nums]:
        # if i==atom_nums+1:
